class MarkersController < ApplicationController
  ...
  private
    ...
    def marker_params
      params.require(:marker).permit(:label, :position)
    end
   
    def find_unspecified_locations
      regions = Photographer.select(:locationspecifier).distinct.order(:locationspecifier).map(&:locationspecifier)
      specified_locations = Marker.select(:label).distinct.map(&:label)
      @unspecified_locations = []
      regions.each { |r| @unspecified_locations<<r unless specified_locations.include? r }
    end
end
